...
from src.optim.multiscale_schedule import MultiScaleSchedule        # NEW
from src.physics.level_transfer import MultiLevelTransfer           # NEW

from src.utils.lanczos import sample_from_simplified_sigma_batched  # NEW
...
class DiffPIRSampler(DDPMSampler):
    ...
    def _transition_step(
        ...
        abar_t = self._extract(self.alphas_cumprod, t, x0_prev.shape)

        sigma2_prev = 1.0 - abar_prev
        sigma2_t = 1.0 - abar_t

        mu_prev = torch.sqrt(abar_prev) * x0_prev

        resid = x - M(mu_prev)
        mu_post = mu_prev + (sigma2_prev / sigma2_t) * MT(resid)
    ...
    def sample(
        ...
        noise=None,
        gt=None,
        num_ddim_steps: int = 100,  # Added for DDIM jump-stepping
        zeta: float = 0.0,          # Added for DDIM stochastic control
    ):
        #self.model = model
        self.lambda_ = lambda_
        ...
        # Generate a quadratic subsequence of timesteps bounded by t_start
        times = np.linspace(0, np.sqrt(t_start), num_ddim_steps) ** 2
        times = np.round(times).astype(int)

        # Force insert transition_ts to guarantee exact alignment at boundaries
        for ts in transition_ts:
            if 0 <= ts <= t_start:
                times = np.append(times, ts)     # ensure transition time steps are included in the sampling sequence
            if 0 <= ts + 1 <= t_start:
                times = np.append(times, ts + 1) # ensure the previous step is also included for smooth transitions
        ...
        for i in range(len(times)):
            ...
            level_t = schedule._level_of_t[t_val]
            level_t_prev = schedule._level_of_t[t_prev_val if t_prev_val >= 0 else 0]

            is_transition = (t_prev_val >= 0) and (level_t != level_t_prev)
            ...
            print(self._gamma(t_val))

            before = 0.04825 * x0   # for plotting
            ...
            plt.imsave(
                f'./outputs/diffpir/{t_val}_before_prox.png',
                x0.detach().clone().squeeze().cpu(),
            ...
            x0_tilde_denorm = self.data_fidelity.prox(x0_denorm, y_t, physics_t, gamma=self._gamma(t_val))
        
            after = x0_tilde_denorm # for plotting
            
            x0 = x0_tilde_denorm / 0.04825
            
            plt.imsave(
                f'./outputs/diffpir/{t_val}_after_prox.png',
                x0.detach().clone().squeeze().cpu(),
                cmap='gray', vmin=0.0, vmax=1.0
            )       
            ...
            if gt is not None:
                gt_level = transfer.projection_between_levels(gt, n_from=0, n_to=level_t)
                fig, axes = plt.subplots(1, 3, figsize=(12, 4))

                axes[0].imshow(gt_level[0, 0].detach().cpu(), cmap='gray', vmin=0.0, vmax=0.04825)
                axes[0].set_title(f"ground truth {tuple(gt_level.shape)}")

                axes[1].imshow(before[0, 0].detach().cpu(), cmap='gray', vmin=0.0, vmax=0.04825)
                axes[1].set_title(f"before prox {tuple(before.shape)}")

                axes[2].imshow(after[0, 0].detach().cpu(), cmap='gray', vmin=0.0, vmax=0.04825)
                axes[2].set_title(f"after prox {tuple(after.shape)}")

                for ax in axes:
                    ax.axis("off")

                plt.tight_layout()
                plt.savefig(f'./outputs/diffpir/{t_val}_compare.png')
                plt.close(fig)
...
def _self_test():
    ...
    sampler = DDPMSampler(num_timesteps=1000, device=device)

    B, C, H, W = 2, 1, 64, 64
    x0 = torch.rand(B, C, H, W, device=device)
    xt = torch.randn_like(x0)
    t = torch.tensor([10, 10], device=device, dtype=torch.long)

    abar_prev = sampler._extract(sampler.alphas_cumprod_prev, t, x0.shape)
    abar_t = sampler._extract(sampler.alphas_cumprod, t, x0.shape)
    sigma2_prev = 1 - abar_prev
    sigma2_t = 1 - abar_t
    alpha_t = sampler._extract(sampler.alphas, t, x0.shape)
    s_t = torch.sqrt(alpha_t)

    mu_prev = torch.sqrt(abar_prev) * x0
    mu_kalman = mu_prev + (sigma2_prev / sigma2_t) * s_t * (xt - s_t * mu_prev)

    c1 = sampler._extract(sampler.posterior_mean_coef1, t, x0.shape)
    c2 = sampler._extract(sampler.posterior_mean_coef2, t, x0.shape)
    mu_ddpm = c1 * x0 + c2 * xt

    print("max abs diff:", (mu_kalman - mu_ddpm).abs().max().item())
